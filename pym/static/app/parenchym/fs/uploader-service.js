...
angular.module('pym.fs').factory('pymFsUploaderService', ['$log', '$upload', '$http', 'RC', 'pymService', 'FILE_STATES', 'FILE_STATE_CAPTIONS', function ($log, $upload, $http, RC, pym, FILE_STATES, FILE_STATE_CAPTIONS) {
    ...
    PymFile.prototype.setState = function (state) {
        ...
        this.isActive = this.state > FILE_STATES.NEW && this.state < FILE_STATES.UPLOAD_OK;
        this.isUploading = this.state === FILE_STATES.UPLOADING;
        this.hasError = this.state > FILE_STATES.UPLOAD_CANCELED && this.state < FILE_STATES.NEW;
    ...
    var UploaderService = {
        ...
        createPymFile: function createPymFile(file) {
         ...
         * @returns {promise}
         */
        upload: function upload(pathStr, file) {
            var uploadConf = {
                url: RC.urls.upload,
                file: file.file,
                data: {
            ...
            p = $upload.upload(uploadConf).success(function (data, status, headers, config) {
         ...
         * @returns {promise}
         */
        validateFiles: function validateFiles(pathStr, fileList) {
            var httpConfig = {},
                postData = {},
                ff = [];
            $log.log('fileList to validate', fileList);
            angular.forEach(fileList, function (v) {
            ...
            postData.files = ff;
            return $http.post(RC.urls.validate_files, postData, httpConfig).then(function (resp) {
                if (resp.data.ok) {
