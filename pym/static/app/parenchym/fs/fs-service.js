...
angular.module('pym.fs').factory('pymFsService', ['$log', '$http', '$q', '$window', 'RC', 'pymService', function ($log, $http, $q, $window, RC, pym) {
    ...
    var FsService = {
        ...
        getPathStr: function getPathStr() {
        ...
        getLeafNode: function getLeafNode() {
         ...
         */
        toggleIncludeDeleted: function toggleIncludeDeleted() {
            var pp = this.path,
                p0 = pp[0];
         ...
         */
        firstLoad: function firstLoad(pathStr) {
            var self = this;
            this.rootPathStr = pathStr;
            $log.log('firstLoad', this.pathToStr(this.path), this.rootPathStr);
            this.browser.loadItems();
            this.tree.initNodes().then(function (resp) {
                self.path = self.tree.path;
                self.browser.path = self.path;
            });
        },

        extractMeta: function extractMeta(nameList) {
            var httpConfig = {},
                postData = {
                names: nameList,
                path: this.pathToStr(this.path)
            };
            return $http.put(RC.urls.extract_meta, postData, httpConfig).then(function (resp) {
                if (resp.data.ok) {
                    // Noop
                }
        ...
        createDirectory: function createDirectory(dirName) {
            var httpConfig = {},
                postData = {
                name: dirName,
                path: this.pathToStr(this.path)
            };
            return $http.post(RC.urls.create_directory, postData, httpConfig).then(function (resp) {
                if (resp.data.ok) {
                    // Noop
                }
                pym.growler.growlAjaxResp(resp.data);
        ...
        deleteItems: function deleteItems(names, reason) {
            var httpConfig = {
                params: {
                    path: this.pathToStr(this.path),
                    names: names,
            ...
            return $http.delete(RC.urls.delete_items, httpConfig).then(function (resp) {
                if (resp.data.ok) {
                    // NOOP
                }
                pym.growler.growlAjaxResp(resp.data);
        ...
        undeleteItems: function undeleteItems(names) {
            var httpConfig = {},
                putData = {
                path: this.pathToStr(this.path),
                names: names
            };
            return $http.put(RC.urls.undelete_items, putData, httpConfig).then(function (resp) {
                if (resp.data.ok) {
                    // NOOP
                }
                pym.growler.growlAjaxResp(resp.data);
        ...
        loadItems: function loadItems() {
            ...
            if (leafNode && leafNode.id <= 0) {
                ...
                if (leafNode.name === 'search') {
                    var searchParams,
                        s = this.globalOptions.search.replace(/^\s+/, '').replace(/\s+$/, ''),
            ...
            return $http.get(RC.urls.load_items, httpConfig).then(function (resp) {
        ...
        loadTree: function loadTree(filter) {
            var httpConfig = {
                params: {
            ...
            return $http.get(RC.urls.load_tree, httpConfig).then(function (resp) {
                if (resp.data.ok) {
                    // noop
                } else {
                        pym.growler.growlAjaxResp(resp.data);
        ...
        loadFsProperties: function loadFsProperties() {
            var httpConfig = { params: {} };
            return $http.get(RC.urls.load_fs_properties, httpConfig).then(function (resp) {
                if (resp.data.ok) {
                    return resp;
                } else {
                    pym.growler.growlAjaxResp(resp.data);
        ...
        loadItemProperties: function loadItemProperties(name) {
            var httpConfig = {
                params: {
                    path: this.pathToStr(this.path),
                    name: name
            ...
            return $http.get(RC.urls.load_item_properties, httpConfig).then(function (resp) {
                if (resp.data.ok) {
                    return resp;
                } else {
                    pym.growler.growlAjaxResp(resp.data);
        ...
        changeItemAttr: function changeItemAttr(itemId, attr, newValue, oldValue) {
            var httpConfig = {},
                putData = {
                id: itemId,
                attr: attr,
            ...
            return $http.put(RC.urls.edit_item, putData, httpConfig).then(function (resp) {
                pym.growler.growlAjaxResp(resp.data);
        ...
        buildDownloadUrl: function buildDownloadUrl(nameOrEntity) {
            ...
            if (angular.isString(nameOrEntity)) {
                ...
                s = $window.location.origin + uu + '/' + entity.location + '/' + entity._name;
                return s;
            }
        },

        pathToStr: function pathToStr(path) {
